...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        fatal("Usage: %s <host> <port> <message> <packets> <size> ...\n", argv[0]);
    }

    char* host = argv[1];
    uint16_t port = read_port(argv[2]);
    uint64_t packets = read_port(argv[3]);
    uint64_t size = read_port(argv[4]);
    ...
    for (uint64_t i = 0; i < packets; i++) {
        ...
        send_message(socket_fd, &send_address, com);
        printf("sent to %s:%u: '%s'\n", client_ip, client_port, com);
